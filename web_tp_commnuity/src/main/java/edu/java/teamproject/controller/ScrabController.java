...
public class ScrabController {
   ...
   @RequestMapping(value = "myScrabList", method = RequestMethod.POST)
   public ResponseEntity<List<Board>> myScrabList(String user_id) {
      logger.info("myScrabList Controller : " + user_id);

      List<Board> list = scrabService.myScrabList(user_id);
      logger.info("myScrabList Board 값 : " + list.toString());
      ResponseEntity<List<Board>> entity = new ResponseEntity<List<Board>>(list, HttpStatus.OK);
      return entity;
   }
